...
export async function create(payload: Partial<IUser>) {
  const result = await api<IBaseResponse<IUser>>({
    url: `${CONST_APIS.CONT_HOST_SERVER}/${CONST_APIS.CONST_VERSION}/${CONST_APIS.CONST_IMAGE}`,
    options: {
      method: 'POST',
...
export async function findOneBySlug(slug: string) {
  const result = await api<IBaseResponse<IUser>>({
    url: `${CONST_APIS.CONT_HOST_SERVER}/${CONST_APIS.CONST_VERSION}/${CONST_APIS.CONST_IMAGE}/${slug}`,
    options: {
      method: 'GET',
      next: {
...
export async function softRemoveMulti(payload: {ids: string[]}) {
  const result = await api<IBaseResponse<IUser>>({
    url: `${CONST_APIS.CONST_VERSION}/${CONST_APIS.CONST_VERSION}/${CONST_APIS.CONST_IMAGE}`,
    options: {
      method: 'DELETE',
      body: JSON.stringify(payload),
    },
