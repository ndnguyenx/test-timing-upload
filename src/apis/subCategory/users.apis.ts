...
export async function create(payload: Partial<IUser>) {
  const result = await api<IBaseResponse<IUser>>({
    url: `${CONST_APIS.CONT_HOST_SERVER}/${CONST_APIS.CONST_VERSION}/${CONST_APIS.CONST_SUBCATEGORY}`,
    options: {
      method: 'POST',
...
export async function findOneBySlug(slug: string) {
  const result = await api<IBaseResponse<IUser>>({
    url: `${CONST_APIS.CONT_HOST_SERVER}/${CONST_APIS.CONST_VERSION}/${CONST_APIS.CONST_SUBCATEGORY}/${slug}`,
    options: {
      method: 'GET',
      next: {
...
export async function update(id: string, payload: Partial<IUser>) {
  const result = await api<IBaseResponse<IUser>>({
    url: `${CONST_APIS.CONT_HOST_SERVER}/${CONST_APIS.CONST_SUBCATEGORY}/${id}`,
    options: {
      method: 'PATCH',
      body: JSON.stringify(payload),
    },
...
export async function softRemoveMulti(payload: {ids: string[]}) {
  const result = await api<IBaseResponse<IUser>>({
    url: `${CONST_APIS.CONST_VERSION}/${CONST_APIS.CONST_VERSION}/${CONST_APIS.CONST_SUBCATEGORY}`,
    options: {
      method: 'DELETE',
      body: JSON.stringify(payload),
    },
